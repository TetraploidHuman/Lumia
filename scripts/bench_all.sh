...
#   SKIP_CPU=1 SKIP_MEMO=1 SKIP_REPR=1 SKIP_CN=1 SKIP_CN_STEP=1 SKIP_CN_EFE=1 SKIP_CN_FUSE=1 SKIP_CN_FORWARD=1 SKIP_CN_STRICT=1
set -euo pipefail
...
if [[ "${SKIP_REPR:-0}" != "1" ]]; then
  echo "######## bench_repr ########"
  if RUNS="$CN_RUNS" bash "$ROOT/scripts/bench_repr.sh"; then
    echo "OK bench_repr"
  else
    echo "FAIL bench_repr" >&2
    fail=1
  fi
  echo
fi
