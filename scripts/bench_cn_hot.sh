...
#
# Compares nested Lumi List[Float] loops (auto-SR'd to RT kernels) vs
...
OUT_DIR="${TMPDIR:-/tmp}/lumi_bench_cn_hot"
mkdir -p "$OUT_DIR"
RUNS="${RUNS:-5}"

echo "== build =="
"$LUMI" build --release examples/bench_cn_hot_kernel.lm -o "$OUT_DIR/kernel"
"$LUMI" build --release examples/bench_cn_hot_naive.lm -o "$OUT_DIR/naive"
