#!/usr/bin/env bash
# Create a minimal Lumi package on disk (remote-friendly; no IDE required).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"

usage() {
  echo "Usage: $0 NAME [PARENT_DIR]" >&2
  echo "  Creates PARENT_DIR/NAME/{Lumi.toml,src/main.lm} (default PARENT_DIR: \$PWD)" >&2
  exit 1
}
