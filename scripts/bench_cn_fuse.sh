...
OUT_DIR="${TMPDIR:-/tmp}/lumia_bench_cn_fuse"
mkdir -p "$OUT_DIR"
RUNS="${RUNS:-5}"

echo "== build =="
"$LUMIA" build --release examples/bench/bench_cn_fuse_kernel.lm -o "$OUT_DIR/kernel"
"$LUMIA" build --release examples/bench/bench_cn_fuse_naive.lm -o "$OUT_DIR/naive"
