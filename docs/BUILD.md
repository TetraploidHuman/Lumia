...
- Pass 接口在 `lumi_opt`：`cse` / `const_fold` / `licm`（Debug+Release，局部消重）+ Release 的 `memo_tf`（有界 `T_f`：Slots / DenseInt；**CSE 前**做建表规划，非 pass 循环内空跑）；`--no-memo`（别名 `--no-memo-l2`）可关 runtime Memo 做对比。运行时 C 符号仍为 `lumi_memo_l2_*`（ABI 冻结）。
  ...
  - **阶段 H（部分）**：`--no-memo-dense`；`--mm ms|arc`（Arc：COW + String/Char/Fun `lumi_heap_*`；free-on-zero；STW full；环候选缓冲安全点 flush）；`LUMI_HEAP_SHARED=1` 进程级成员镜像；`LUMI_GC_MARK_THREADS>1` STW 并行 mark。
...
| **仍待** | Bacon–Rajan / trial-delete；多 mutator 共享 nursery（young/old 向量仍 TLS） |
