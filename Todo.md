...
- [x] **类型类 `trait` / `instance` / `requires`**：显式 instance / UFCS / 派生 / Hash opt-in / 多态方法经单态解析已接线；**运行时字典** `lumi_dict_register/lookup`（Show/Eq/Ord/Hash/Num）在 `main` 启动注册 mangled instance（动态分发可查表；热路径仍直连单态）。
...
- [x] **Float 作 Map/Set 键与 `lumi_eq`**：`TYPE_MAP_F64` / `TYPE_SET_F64` + IEEE `float_key_eq`/`float_key_hash`（±0 碰撞、NaN 永不命中）；codegen 在 Float 键/`set` 时 `lumi_ensure_*_f64`；e2e `float_map_keys`。
- [x] **Float 结构相等（List / ADT / Map 值）**：`TYPE_LIST_F64`、`TYPE_MAP_VF64` / `TYPE_MAP_F64V`；ADT 经 `lumi_adt_eq(float_mask)`（按**对象实际 size**，非 type-param 元数）；e2e `float_struct_eq` / `adt_float_eq`。
- [x] **嵌套 Float ADT 的 eq/hash**：ADT header `_pad` 存 per-field Float mask；`lumi_eq` / `hash_value` / `lumi_adt_eq` 读 mask（IEEE）；codegen `lumi_adt_set_float_mask`；e2e `nested_float_adt_eq`。
- [x] **ADT 字段 GC**：`mark` 对 `TYPE_ADT` 按 `_pad` 跳过 Float 槽。
- [x] **`lumi_show` 集合格式**：List/Map/Set 递归展示元素（`[…]` / `{k: v}` / `#{…}`）；积/和 ADT 仍为 `#tag(…)`。
- [x] **标量路径 `lumi_eq` 未装箱 Float（契约锁定）**：非堆 i64 仍 bit 短路（`lumi_rt`/`lumi_abi::float_contract` 单测）；标量 `==` 走 codegen `fcmp`；IEEE ±0/NaN 仅容器 typed eq / `fcmp`。嵌套无标签 Float 位不在 `lumi_eq` 保证范围。
...
- [x] **陷阱栈追踪**（DESIGN §2）：codegen `lumi_frame_push/pop` + `trap_abort` 打印 Lumi 调用栈；musttail 前 pop。
...
- [x] **自定义 `Eq.eq` 与 Map/Set 键相等分裂**：有 `instance Hash` 的 ADT 上 `==` 强制走 `lumi_eq`（与 Map/Set 一致），忽略发散的 `__Eq_*_eq`；e2e `eq_hash_consistent`。
...
- [x] **逃逸分析 → 栈分配 / 多表示 List·Map·Set**：Lit* / LitAdt / 纯 callee 摘要已接线；**晋升** `lumi_list_promote`（concat 空列表恒等返回前将栈 LitList 升堆）。更多表示仍可扩展。
...
- [x] **`std/` 可执行正文**：`std.option` / `std.result` / `std.string` / `std.io` 均为 Source 并经 loader 内联；string/io 薄包装 `lumi_*` runtime foreign（对象 ABI）；`println`/`assert` 仍降为编译器 builtin。

## 工具链

- [x] **`lumi doc`**：CLI 生成 Markdown（`///`、公开 `val`/`type`/`foreign`、`@exports`）；`priv` 默认隐藏。
- [~] **并发 GC**：分代 STW minor 不变；增量并发 full mark（MS）；并行 drain；`--mm arc`（String/Char/Fun heap RC；STW full；环候选安全点 flush；**overlay/slice parent retain**；sweep 先卸 `HEAP_*` 再 `release_children`）。`LUMI_HEAP_SHARED=1` 成员镜像（测试 TLS；跨线程 free 不支持）。Young/old 仍 TLS；trial-delete / 共享 nursery 仍待。

## 架构清理（已落地，详见 git 历史）

- 共享前端：`lumi_ty::typecheck_hir`；`lumi` 为 lib+bin（`check_program` / LSP / CLI）。
- `Builtin::info` 元数据（按 family 表 + `may_capture`/`result_heap` 同表）；`builtin_surface`；codegen `CodegenError` + 子状态；`lumi_abi::float_contract`。
...
- **Trait mangling**：`lumi_hir::mangle_trait_method` 统一 HIR/codegen；`from_method`↔`display_name` 一致性单测。
- **CoreModule::with_functions** / **CodegenTypeTables**；opt 管线 `PipelinePass` 免 `Box<dyn Pass>`；Memo Rust 侧以 `MEMO_TF_*` 为准（C ABI 仍 `lumi_memo_l2_*`）。
