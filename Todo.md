...
- [ ] **单态化管线**：Float 特化 `__lam_*$Float` 与顶层命名 `{name}$Float`（`poly_inc` / `poly_add1` / `poly_top_dbl`；顶层 Fun 在 generalize 前摘掉递归占位以得 scheme）；ADT `instance Num` 的 `+`/`*` 已接线（`trait_num`）；完整多体 / 非 Num 泛型仍待。恒等另有 call-site 启发式（`poly_id`）。
