...
/** Creates `Lumia.toml` + `src/main.lm` (aligned with `lumia pkg init` + hello template). */
object LumiaProjectScaffold {
    ...
    fun hasManifest(baseDir: VirtualFile): Boolean = baseDir.findChild("Lumia.toml") != null
    ...
    fun createOnDisk(projectDir: String, packageName: String) {
        ...
        File(dir, "Lumia.toml").writeText(renderToml(pkg))
     ...
     * @throws IllegalStateException if `Lumia.toml` already exists
     */
    fun create(baseDir: VirtualFile, packageName: String): VirtualFile {
        ...
        return WriteAction.compute<VirtualFile, RuntimeException> {
            if (baseDir.findChild("Lumia.toml") != null) {
                throw IllegalStateException("Lumia.toml already exists in ${baseDir.path}")
            }
            val toml = baseDir.createChildData(this, "Lumia.toml")
