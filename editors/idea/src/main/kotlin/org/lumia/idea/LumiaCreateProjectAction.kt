 ...
 */
class LumiaCreateProjectAction :
    com.intellij.openapi.actionSystem.AnAction(
        "New Lumia Project…",
        "Create a Lumia package (Lumia.toml + src/main.lm) and open it",
        LumiaIcons.FILE,
    ),
    DumbAware {

    override fun actionPerformed(e: com.intellij.openapi.actionSystem.AnActionEvent) {
        val dialog = LumiaCreateProjectDialog()
        if (!dialog.showAndGet()) return
        val projectPath = dialog.projectPath
        val packageName = dialog.packageName
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                LumiaProjectScaffold.createOnDisk(projectPath, packageName)
                ApplicationManager.getApplication().invokeLater {
                    val opened =
                        ProjectUtil.openOrImport(Path.of(projectPath), e.project, true)
                    if (opened != null) {
                        openMainIfPresent(opened)
                        LumiaProjectActivity.kick(opened, "createProject")
                    }
                }
            } catch (ex: Exception) {
                ApplicationManager.getApplication().invokeLater {
                    com.intellij.openapi.ui.Messages.showErrorDialog(
                        e.project,
                        ex.message ?: "Failed to create Lumia project",
                        "Lumia",
                    )
                }
            }
...
private class LumiaCreateProjectDialog : DialogWrapper(null) {
    ...
    init {
        title = "New Lumia Project"
        locationField.text = defaultProjectsDir()
        locationField.addBrowseFolderListener(
            "Select Parent Directory",
            "Choose where the new Lumia project folder will be created",
            null,
            null,
        )
        init()
    ...
    override fun doValidate(): ValidationInfo? {
        ...
        val pkgErr = LumiaProjectScaffold.validatePackageName(packageName)
