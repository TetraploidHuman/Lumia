...
/// Codegen calling convention for builtins that are a direct `lumi_*` call.
///
...
pub enum BuiltinEmit {
    ...
    ObjI64I64Ptr,
    /// `(obj ptr, i64)` + codegen Option some/none tags → scalar i64 (`lumi_get`).
    ObjI64OptionTags,
}
...
pub struct BuiltinInfo {
    ...
    /// Primary `lumi_*` runtime symbol when emission is a direct call.
    ...
    /// seeds Show operands separately so they are heap-rooted for `lumi_show`.
...
pub(crate) const ENS_LIST_APPEND: &[(u8, &str)] = &[(1, lumi_abi::ENSURE_LIST_F64)];
pub(crate) const ENS_SET_INSERT: &[(u8, &str)] = &[(1, lumi_abi::ENSURE_SET_F64)];
pub(crate) const ENS_MAP_SET: &[(u8, &str)] = &[
    (1, lumi_abi::ENSURE_MAP_F64),
    (2, lumi_abi::ENSURE_MAP_VF64),
];
...
impl Builtin {
    ...
    /// Shared family used by `lumi_ty` and `lumi_codegen` routers.
    ...
    /// Primary runtime symbol when emission is a direct `lumi_*` call.
