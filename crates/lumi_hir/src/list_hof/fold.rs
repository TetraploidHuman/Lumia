...
impl FoldAssocExpr for lumi_syntax::Expr {
    fn match_add_mul_vars(&self, a: &str, b: &str) -> bool {
        match self {
            lumi_syntax::Expr::Binary {
                ...
                (lumi_syntax::Expr::Ident(l, _), lumi_syntax::Expr::Ident(r, _)) => {
    ...
    fn nested_for_assoc(&self) -> Option<&Self> {
        match self {
            lumi_syntax::Expr::Block { tail, .. } => tail.as_deref(),
            lumi_syntax::Expr::Lambda { body, .. } => Some(body),
            _ => None,
        }
    }
}
...
pub(crate) fn syntax_fold_body_is_associative(body: &lumi_syntax::Expr, a: &str, b: &str) -> bool {
