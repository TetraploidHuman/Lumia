...
pub(crate) fn check_expr_matches(
    ...
    match e {
        ...
        S::Block { stmts, tail, .. } => {
            for s in stmts {
                match s {
                    lumi_syntax::Stmt::Val { expr, .. }
                    | lumi_syntax::Stmt::Var { expr, .. }
                    | lumi_syntax::Stmt::Assign { expr, .. }
                    | lumi_syntax::Stmt::Expr(expr) => {
                        check_expr_matches(expr, ctors, adts, products)?
                    }
                    lumi_syntax::Stmt::ForIn { iter, body, .. }
                    ...
                    lumi_syntax::Stmt::Break(_) | lumi_syntax::Stmt::Continue(_) => {}
                }
            }
            if let Some(t) = tail {
        ...
        S::Interp { parts, .. } => {
            for p in parts {
                if let lumi_syntax::InterpPart::Expr(e) = p {
                    check_expr_matches(e, ctors, adts, products)?;
                }
            }
        }
