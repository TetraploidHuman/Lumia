...
fn scan_type_decls(m: &lumi_syntax::Module) -> TypeScan {
    ...
    for item in &m.items {
        if let lumi_syntax::Item::Type(t) = item {
            match &t.kind {
                lumi_syntax::TypeKind::Sum(variants) => {
                    let mut vs = Vec::new();
                ...
                lumi_syntax::TypeKind::Product(fields) => {
...
fn collect_instances(
    ...
    for item in &m.items {
        match item {
            lumi_syntax::Item::Trait(t) => {
                trait_requires.insert(t.name.clone(), t.requires.clone());
            }
            lumi_syntax::Item::Instance(i) => {
            ...
            for req in reqs {
                if !instances.contains(&(req.clone(), ty.clone())) {
                    let span = m
                        ...
                        .unwrap_or_else(lumi_syntax::Span::dummy);
                    return Err(LowerError {
                        message: format!(
...
fn collect_toplevel_funs(m: &lumi_syntax::Module) -> HashSet<String> {
        ...
        match item {
            lumi_syntax::Item::Val(v) => {
                ...
                let is_fun = v.params.is_some()
                    || matches!(
                        v.body,
                        lumi_syntax::Expr::Lambda { .. } | lumi_syntax::Expr::Block { .. }
                    );
                if is_fun {
                    toplevel_funs.insert(v.name.clone());
                }
            }
            lumi_syntax::Item::Foreign(f) => {
...
fn collect_fold_assoc(m: &lumi_syntax::Module) -> HashSet<String> {
    let mut toplevel_fold_assoc = HashSet::default();
    for item in &m.items {
        if let lumi_syntax::Item::Val(v) = item {
            ...
            } else if let lumi_syntax::Expr::Lambda { params, body, .. } = &v.body {
                if params.len() == 2
                    && crate::list_hof::syntax_fold_body_is_associative(
...
pub fn lower_module_with_options(
    ...
    let mut trait_defaults: HashMap<String, HashMap<String, lumi_syntax::ValItem>> =
        HashMap::default();
    ...
    for item in &m.items {
        if let lumi_syntax::Item::Trait(t) = item {
    ...
    for item in &m.items {
        match item {
            ...
            lumi_syntax::Item::Type(_) | lumi_syntax::Item::Trait(_) => {}
            lumi_syntax::Item::Instance(i) => {
