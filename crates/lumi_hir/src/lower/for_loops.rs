...
pub(crate) fn lower_for_in(
    ctx: &LowerCtx,
    binding: &lumi_syntax::ForBinding,
    iter: &lumi_syntax::Expr,
    body: &lumi_syntax::Expr,
    span: Span,
) -> Expr {
    let body_e = lower_expr(ctx, body);
    match binding {
        lumi_syntax::ForBinding::Pair(k, v) => {
            let lowered = lower_expr(ctx, iter);
            // Map → MapItems; already a List[(K,V)] (listOf / items / sortBy) → as-is.
            // Runtime `lumi_map_items` is also identity on List as a safety net.
        ...
        lumi_syntax::ForBinding::Name(name) => {
