...
//! - [`check_expr_effects`] walks into lambda bodies under an *effect* context
//!   (so deferred IO is allowed) while still rejecting eager IO in a pure outer body.
...
pub fn check_effect_boundaries(typed: &TypedModule) -> Result<(), TypeError> {
    for item in &typed.module.items {
        if let Item::Fun(f) = item {
            ...
            // Single walk: pure bodies use `in_effect_ctx = false` (rejects eager IO;
            // nested lambdas still enter an effect context for deferred IO).
            check_expr_effects(
                &f.body,
                fun_is_effectful,
                &typed.fun_types,
