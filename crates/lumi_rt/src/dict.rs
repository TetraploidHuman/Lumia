...
#[no_mangle]
pub extern "C" fn lumi_dict_register(trait_id: i32, type_name: *const u8, method: *const ()) {
    let name = cstr_name(type_name);
    if name.is_empty() || method.is_null() {
        trap_abort("lumi: dict_register invalid args");
    }
    with_dicts(|m| {
...
#[no_mangle]
pub extern "C" fn lumi_dict_lookup(trait_id: i32, type_name: *const u8) -> *const () {
    let name = cstr_name(type_name);
    with_dicts(|m| {
        m.get(&(trait_id, name))
            .copied()
...
#[no_mangle]
pub extern "C" fn lumi_dict_show(trait_id: i32, type_name: *const u8, value: i64) -> *mut u8 {
    let f = lumi_dict_lookup(trait_id, type_name);
    if f.is_null() {
...
mod tests {
    ...
    fn register_and_lookup_show() {
        let name = b"Point\0";
        lumi_dict_register(TRAIT_SHOW, name.as_ptr(), show_stub as *const ());
        let p = lumi_dict_lookup(TRAIT_SHOW, name.as_ptr());
        assert!(!p.is_null());
        let got = lumi_dict_show(TRAIT_SHOW, name.as_ptr(), 42);
