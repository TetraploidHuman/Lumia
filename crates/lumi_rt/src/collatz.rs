...
#[no_mangle]
pub extern "C" fn lumi_collatz_total(limit: i64) -> i64 {
    if limit < 1 {
        return 0;
    }
...
#[no_mangle]
pub extern "C" fn lumi_collatz_strided(start: i64, limit: i64, stride: i64) -> i64 {
    if stride == 1 && start <= 1 {
        return lumi_collatz_total(limit);
    }
...
mod tests {
    ...
    fn bench_cpu_collatz_checksum() {
        assert_eq!(lumi_collatz_total(250_000), 29_265_567);
    }

    #[test]
    fn bench_cpu_collatz_2_5m() {
        assert_eq!(lumi_collatz_total(2_500_000), 352_279_148);
    }

    #[test]
    fn bench_cpu_collatz_strided() {
        assert_eq!(lumi_collatz_strided(1, 3_000_000, 3), 142_794_532);
    }

    #[test]
    fn strided_1_matches_total() {
        assert_eq!(
            lumi_collatz_strided(1, 10_000, 1),
            lumi_collatz_total(10_000)
        );
    }

    #[test]
    fn collatz_edges_and_small_oracle() {
        assert_eq!(lumi_collatz_total(0), 0);
        assert_eq!(lumi_collatz_total(-5), 0);
        assert_eq!(lumi_collatz_strided(1, 0, 3), 0);
        assert_eq!(lumi_collatz_strided(10, 5, 1), 0);
        ...
        for &(start, limit, stride) in &[
            ...
            assert_eq!(
                lumi_collatz_strided(start, limit, stride),
