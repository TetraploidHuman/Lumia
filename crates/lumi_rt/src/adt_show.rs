//! ADT Show-kind registry — constructor names for recursive `lumi_show`.
//!
//! Heap ADTs pack a kind id into `type_id` bits `[31:16]` ([`lumi_abi::adt_type_id`]).
...
#[no_mangle]
pub extern "C" fn lumi_adt_register_show(kind: u32, names: *const *const u8, n: i64) {
    if kind == 0 || names.is_null() || n < 0 {
        trap_abort("lumi: adt_register_show invalid args");
    }
    let n = n as usize;
