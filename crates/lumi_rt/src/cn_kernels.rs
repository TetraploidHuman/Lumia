...
#[no_mangle]
pub extern "C" fn lumi_cn_nucleus_step(
    mu: *mut u8,
    enc_w: *mut u8,
    pred_w: *mut u8,
    bottom_up: *mut u8,
    ...
    if size < 1 || size as usize > MAX_DIM {
        trap_abort("lumi: cn nucleus_step size out of range");
    }
    let n = size as usize;
...
#[no_mangle]
pub extern "C" fn lumi_cn_hebbian(
    w: *mut u8,
    u: *mut u8,
    v: *mut u8,
    mask: *mut u8,
    ...
    if from_size < 1 || to_size < 1 {
        trap_abort("lumi: cn hebbian non-positive dims");
    }
    if from_size as usize > MAX_DIM || to_size as usize > MAX_DIM {
        trap_abort("lumi: cn hebbian dim out of range");
    }
...
#[no_mangle]
pub extern "C" fn lumi_cn_project_clamp(
    from_size: i64,
    to_size: i64,
    w: *mut u8,
    x: *mut u8,
    ...
    let _gc = GcInhibitGuard::enter();
    if from_size < 1 || to_size < 1 {
        trap_abort("lumi: cn project_clamp non-positive dims");
    }
    if from_size as usize > MAX_DIM || to_size as usize > MAX_DIM {
        trap_abort("lumi: cn project_clamp dim out of range");
    }
    let m = from_size as usize;
    let n = to_size as usize;
    let w = force_f64(w);
...
#[no_mangle]
pub extern "C" fn lumi_cn_backproj_clamp(
    m: i64,
    n: i64,
    w: *mut u8,
    x: *mut u8,
    ...
    let _gc = GcInhibitGuard::enter();
    if m < 1 || n < 1 {
        trap_abort("lumi: cn backproj_clamp non-positive dims");
    }
    if m as usize > MAX_DIM || n as usize > MAX_DIM {
        trap_abort("lumi: cn backproj_clamp dim out of range");
    }
    let mm = m as usize;
    let nn = n as usize;
    let w = force_f64(w);
...
#[no_mangle]
pub extern "C" fn lumi_cn_axpy_clamp(y: *mut u8, alpha: f64, x: *mut u8, clip: f64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let x = force_f64(x);
    let y = ensure_unique_f64(y);
    let n = list_len_of(x);
...
#[no_mangle]
pub extern "C" fn lumi_cn_argmax(xs: *mut u8) -> i64 {
    let xs = force_f64(xs);
    unsafe {
...
#[no_mangle]
pub extern "C" fn lumi_cn_cluster_rates(
    potential: *mut u8,
    input: *mut u8,
    output: *mut u8,
    size: i64,
    ...
    let _gc = GcInhibitGuard::enter();
    if size < 1 || size as usize > MAX_DIM {
        trap_abort("lumi: cn cluster_rates size out of range");
    }
    if n_clusters < 1 {
        trap_abort("lumi: cn cluster_rates n_clusters < 1");
    }
    let n = size as usize;
...
#[no_mangle]
pub extern "C" fn lumi_cn_learn_generative(
    enc_w: *mut u8,
    pred_w: *mut u8,
    mu: *mut u8,
    err: *mut u8,
    ...
    let _gc = GcInhibitGuard::enter();
    if size < 1 || size as usize > MAX_DIM {
        trap_abort("lumi: cn learn_generative size out of range");
    }
    let n = size as usize;
    let mu = force_f64(mu);
    ...
    unsafe {
        ...
        if std::ptr::eq(enc, ep as *mut f64)
            ...
            || std::ptr::eq(enc, pw)
        {
            trap_abort("lumi: cn learn_generative aliased buffers");
        }
...
#[no_mangle]
pub extern "C" fn lumi_cn_update_state(
    mu: *mut u8,
    enc_w: *mut u8,
    err: *mut u8,
    size: i64,
    ...
    let _gc = GcInhibitGuard::enter();
    if size < 1 || size as usize > MAX_DIM {
        trap_abort("lumi: cn update_state size out of range");
    }
    let n = size as usize;
    let enc_w = force_f64(enc_w);
    let err = force_f64(err);
...
fn force_f64(list: *mut u8) -> *mut u8 {
    ...
    if list.is_null() {
        trap_abort("lumi: cn on null list");
    }
    if !list_float_elems(list) {
        trap_abort("lumi: cn expects List[Float]");
    }
    list
}
...
fn require_unique_f64(list: *mut u8, what: &str) -> *mut u8 {
    ...
    trap_abort(&format!(
        "lumi: {what} must be uniquely owned (multi-buffer COW would drop updates)"
    ));
}
...
mod tests {
    ...
    fn get_f(list: *mut u8, i: i64) -> f64 {
        f64::from_bits(lumi_list_get(list, i) as u64)
    }
    ...
    fn nucleus_identity_encoder() {
        ...
        let err = lumi_list_f64_zeros(size);
        let pred = lumi_list_f64_zeros(size);
        let mu = lumi_cn_nucleus_step(mu, enc, pred_w, bu, td, err, pred, size, 0.5, 1.0, 10.0);
    ...
    fn hebbian_outer_and_mask() {
        ...
        let w = lumi_cn_hebbian(w, u, v, mask, 2, 2, 1.0, 10.0, 0.0, 1e-3);
    ...
    fn cluster_rates_square_relu() {
        ...
        let out = lumi_cn_cluster_rates(pot, x, out, 4, 2, 0.9, 0.5);
    ...
    fn learn_generative_outer_products() {
        ...
        let enc = lumi_cn_learn_generative(enc, pred, mu, err, size, 1.0, 10.0, 0.0, 1.0);
    ...
    fn update_state_encoder_step() {
        ...
        let mu = lumi_cn_update_state(mu, enc, err, size, 0.5, 1.0, 10.0);
        // δ = err; μ += 0.5·δ → [1, 0]
