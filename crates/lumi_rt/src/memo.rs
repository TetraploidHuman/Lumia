...
/// Caps live in `lumi_abi` (`MEMO_TF_*`). C entry points stay
/// `lumi_memo_l2_*` for ABI stability (DESIGN vocabulary is `T_f`, not L2).
...
#[no_mangle]
pub extern "C" fn lumi_memo_l2_lookup(
    fun_id: i64,
    nargs: i64,
    a0: i64,
    a1: i64,
...
#[no_mangle]
pub extern "C" fn lumi_memo_l2_store(
    fun_id: i64,
    nargs: i64,
    a0: i64,
    a1: i64,
...
#[no_mangle]
pub extern "C" fn lumi_memo_l2_misses() -> i64 {
    MEMO_TF.with(|t| t.borrow().iter().map(|x| x.misses.get() as i64).sum())
}

#[no_mangle]
pub extern "C" fn lumi_memo_l2_reset() {
    MEMO_TF.with(|t| {
...
#[no_mangle]
pub extern "C" fn lumi_memo_idx_lookup(fun_id: i64, key: i64, out_result: *mut i64) -> i64 {
    if fun_id < 0
...
#[no_mangle]
pub extern "C" fn lumi_memo_idx_store(fun_id: i64, key: i64, result: i64) {
...
#[no_mangle]
pub extern "C" fn lumi_memo_idx_hits() -> i64 {
    MEMO_IDX.with(|t| {
        t.borrow()
            .iter()
...
#[no_mangle]
pub extern "C" fn lumi_memo_idx_misses() -> i64 {
    MEMO_IDX.with(|t| {
        t.borrow()
            .iter()
            .filter_map(|x| x.as_ref())
...
#[no_mangle]
pub extern "C" fn lumi_memo_idx_reset() {
    MEMO_IDX.with(|t| {
