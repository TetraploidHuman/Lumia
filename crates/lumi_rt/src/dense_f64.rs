...
#[no_mangle]
pub extern "C" fn lumi_list_f64_zeros(n: i64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    if n < 0 {
        trap_abort("lumi: list_f64_zeros negative length");
    }
    if n == 0 {
        return crate::list::lumi_ensure_list_f64(crate::list::lumi_list_empty());
    }
    unsafe {
        let dest = lumi_alloc(list_payload_bytes(n), TYPE_LIST_F64);
        if dest.is_null() {
            trap_abort("lumi: list_f64_zeros OOM");
        }
...
#[no_mangle]
pub extern "C" fn lumi_f64_fill(xs: *mut u8, v: f64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
...
#[no_mangle]
pub extern "C" fn lumi_f64_scale(xs: *mut u8, alpha: f64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let xs = ensure_unique_f64(xs);
    unsafe {
        let (p, n) = f64_elems_mut(xs);
...
#[no_mangle]
pub extern "C" fn lumi_f64_sqrt(x: f64) -> f64 {
    x.sqrt()
}

/// Scalar `eˣ` (for hand-written softmax that SR into `lumi_f64_softmax`).
#[no_mangle]
pub extern "C" fn lumi_f64_exp(x: f64) -> f64 {
    x.exp()
}

/// Scalar `sin(x)` (radians).
#[no_mangle]
pub extern "C" fn lumi_f64_sin(x: f64) -> f64 {
    x.sin()
}

/// Scalar `cos(x)` (radians).
#[no_mangle]
pub extern "C" fn lumi_f64_cos(x: f64) -> f64 {
    x.cos()
}

/// Scalar `atan2(y, x)`.
#[no_mangle]
pub extern "C" fn lumi_f64_atan2(y: f64, x: f64) -> f64 {
    y.atan2(x)
}

/// Scalar `hypot(x, y)` = √(x²+y²).
#[no_mangle]
pub extern "C" fn lumi_f64_hypot(x: f64, y: f64) -> f64 {
    x.hypot(y)
}

/// `out[i] = a[i] * b[i]` (same length). Returns `out`.
#[no_mangle]
pub extern "C" fn lumi_f64_mul(out: *mut u8, a: *mut u8, b: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let a = force_f64(a);
    let b = force_f64(b);
...
/// `out[i] = a[i] + b[i]` (same length). Returns `out`.
#[no_mangle]
pub extern "C" fn lumi_f64_add(out: *mut u8, a: *mut u8, b: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let a = force_f64(a);
    let b = force_f64(b);
    let out = ensure_unique_f64(out);
...
/// Euclidean L2 norm.
#[no_mangle]
pub extern "C" fn lumi_f64_l2_norm(xs: *mut u8) -> f64 {
    let xs = force_f64(xs);
    unsafe {
...
#[no_mangle]
pub extern "C" fn lumi_f64_sum_sq(xs: *mut u8) -> f64 {
    let xs = force_f64(xs);
    unsafe {
        let (p, n) = f64_elems(xs);
...
#[no_mangle]
pub extern "C" fn lumi_f64_mean(xs: *mut u8) -> f64 {
    let xs = force_f64(xs);
    unsafe {
        let (p, n) = f64_elems(xs);
        if n == 0 {
...
#[no_mangle]
pub extern "C" fn lumi_f64_std(xs: *mut u8) -> f64 {
    let xs = force_f64(xs);
    unsafe {
        let (p, n) = f64_elems(xs);
        if n == 0 {
...
#[no_mangle]
pub extern "C" fn lumi_f64_softmax(xs: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let xs = ensure_unique_f64(xs);
    unsafe {
        let (p, n) = f64_elems_mut(xs);
...
#[no_mangle]
pub extern "C" fn lumi_f64_l2_normalize(xs: *mut u8, eps: f64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let xs = ensure_unique_f64(xs);
    unsafe {
        let (p, n) = f64_elems_mut(xs);
...
#[no_mangle]
pub extern "C" fn lumi_f64_clamp(xs: *mut u8, lo: f64, hi: f64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let xs = ensure_unique_f64(xs);
    unsafe {
        let (p, n) = f64_elems_mut(xs);
...
#[no_mangle]
pub extern "C" fn lumi_f64_gemv(m: i64, n: i64, a: *mut u8, x: *mut u8, y: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    if m < 0 || n < 0 {
        trap_abort("lumi: gemv negative dims");
    }
    let a = force_f64(a);
    let x = force_f64(x);
...
#[no_mangle]
pub extern "C" fn lumi_f64_gemv_t(m: i64, n: i64, a: *mut u8, x: *mut u8, y: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    if m < 0 || n < 0 {
        trap_abort("lumi: gemv_t negative dims");
    }
    let a = force_f64(a);
    let x = force_f64(x);
    let y = ensure_unique_f64(y);
...
#[no_mangle]
pub extern "C" fn lumi_f64_addmm(
    m: i64,
    n: i64,
    w: *mut u8,
    u: *mut u8,
    ...
    let _gc = GcInhibitGuard::enter();
    if m < 0 || n < 0 {
        trap_abort("lumi: addmm negative dims");
    }
    let u = force_f64(u);
    let v = force_f64(v);
...
#[no_mangle]
pub extern "C" fn lumi_f64_axpy(y: *mut u8, alpha: f64, x: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let x = force_f64(x);
    let y = ensure_unique_f64(y);
    let n = list_len_of(x);
...
#[no_mangle]
pub extern "C" fn lumi_f64_sub(out: *mut u8, a: *mut u8, b: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    let a = force_f64(a);
    let b = force_f64(b);
    let out = ensure_unique_f64(out);
...
#[no_mangle]
pub extern "C" fn lumi_f64_copy(dst: *mut u8, src: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
...
#[no_mangle]
pub extern "C" fn lumi_f64_checksum(xs: *mut u8) -> i64 {
    let xs = force_f64(xs);
    unsafe {
        let (p, n) = f64_elems(xs);
        let mut s = 0.0_f64;
...
fn force_f64(list: *mut u8) -> *mut u8 {
    ...
    if list.is_null() {
        trap_abort("lumi: dense f64 on null list");
    }
    if !list_float_elems(list) {
        trap_abort("lumi: dense f64 expects List[Float]");
    }
    list
}
...
fn clone_f64_list(list: *mut u8) -> *mut u8 {
    unsafe {
        ...
        let dest = lumi_alloc(list_payload_bytes(n), TYPE_LIST_F64);
        if dest.is_null() {
            trap_abort("lumi: dense f64 clone OOM");
        }
...
fn require_len(list: *mut u8, expect: i64, what: &str) {
    ...
    if n != expect {
        trap_abort(&format!("lumi: {what} len {n} != {expect}"));
    }
}
...
mod tests {
    use super::*;
    use crate::list::{lumi_list_get, lumi_list_len, lumi_list_retain};
    ...
    fn from_slice(xs: &[f64]) -> *mut u8 {
        let p = lumi_list_f64_zeros(xs.len() as i64);
        unsafe {
    ...
    fn scale_mul_add() {
        ...
        let a = lumi_f64_scale(a, 0.5);
        assert_eq!(lumi_list_get(a, 0), bits(0.5));
        let out = lumi_list_f64_zeros(4);
        let out = lumi_f64_mul(out, a, b);
        assert_eq!(lumi_list_get(out, 1), bits(2.0));
        let out2 = lumi_list_f64_zeros(4);
        let out2 = lumi_f64_add(out2, a, b);
        assert_eq!(lumi_list_get(out2, 0), bits(2.5));
    }

    #[test]
    fn zeros_and_fill() {
        let xs = lumi_list_f64_zeros(3);
        assert_eq!(lumi_list_len(xs), 3);
        assert_eq!(lumi_list_get(xs, 0), bits(0.0));
        let xs = lumi_f64_fill(xs, 2.5);
        assert_eq!(lumi_list_get(xs, 1), bits(2.5));
    }
    ...
    fn gemv_matches_naive() {
        ...
        let y = lumi_list_f64_zeros(3);
        let y = lumi_f64_gemv(3, 2, a, x, y);
        assert_eq!(lumi_list_get(y, 0), bits(5.0));
        assert_eq!(lumi_list_get(y, 1), bits(11.0));
        assert_eq!(lumi_list_get(y, 2), bits(17.0));
    }
    ...
    fn gemv_t_and_addmm() {
        ...
        let y = lumi_list_f64_zeros(2);
        let y = lumi_f64_gemv_t(2, 2, a, x, y);
        // Aᵀ @ [1,1] = [1+3, 2+4] = [4,6]
        assert_eq!(lumi_list_get(y, 0), bits(4.0));
        assert_eq!(lumi_list_get(y, 1), bits(6.0));

        let w = lumi_list_f64_zeros(4);
        let u = from_slice(&[1.0, 2.0]);
        let v = from_slice(&[3.0, 4.0]);
        let w = lumi_f64_addmm(2, 2, w, u, v, 1.0);
        // [[3,4],[6,8]]
        assert_eq!(lumi_list_get(w, 0), bits(3.0));
        assert_eq!(lumi_list_get(w, 1), bits(4.0));
        assert_eq!(lumi_list_get(w, 2), bits(6.0));
        assert_eq!(lumi_list_get(w, 3), bits(8.0));
    }

    #[test]
    fn normalize_and_cow() {
        let xs = from_slice(&[3.0, 4.0]);
        let xs = lumi_f64_l2_normalize(xs, 0.0);
        assert!((lumi_f64_l2_norm(xs) - 1.0).abs() < 1e-12);

        let a = from_slice(&[1.0, 0.0]);
        lumi_list_retain(a);
        let b = lumi_f64_fill(a, 9.0);
        // Shared → COW; original retained binding keeps old bits.
        assert_ne!(a, b);
        assert_eq!(lumi_list_get(a, 0), bits(1.0));
        assert_eq!(lumi_list_get(b, 0), bits(9.0));
    }
    ...
    fn nucleus_scale_gemv_checksum() {
        ...
        let m = 16i64;
        let n = 32i64;
        let a = lumi_list_f64_zeros(m * n);
        let x = lumi_list_f64_zeros(n);
        let y = lumi_list_f64_zeros(m);
        unsafe {
        ...
        let y = lumi_f64_gemv(m, n, a, x, y);
        assert_eq!(lumi_f64_checksum(y), 2261);
    }
}
