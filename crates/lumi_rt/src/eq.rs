...
#[no_mangle]
pub extern "C" fn lumi_adt_eq(a: i64, b: i64, float_mask: i64) -> i64 {
    let pa = a as *mut u8;
    let pb = b as *mut u8;
    if !is_heap_payload(pa) || !is_heap_payload(pb) {
...
mod tests {
    ...
    fn scalar_non_heap_eq_is_bit_identity() {
        assert_eq!(lumi_eq(0, 0), 1);
        assert_eq!(lumi_eq(1, 2), 0);
        let pos0 = 0.0f64.to_bits() as i64;
        let neg0 = (-0.0f64).to_bits() as i64;
        assert_ne!(pos0, neg0);
        assert_eq!(lumi_eq(pos0, neg0), 0);
    }
}
