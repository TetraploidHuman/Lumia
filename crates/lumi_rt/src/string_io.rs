...
use crate::show::lumi_show;
use lumi_abi::{is_list_tid, is_map_tid, is_set_tid};

#[no_mangle]
pub extern "C" fn lumi_println_int(n: i64) {
...
#[no_mangle]
pub extern "C" fn lumi_read_stdin() -> *mut u8 {
    let mut buf = Vec::new();
    ...
    loop {
        let n = match stdin.read(&mut chunk) {
            Ok(0) => break,
            Ok(n) => n,
            Err(e) => trap_abort(&format!("lumi: stdin read error: {e}")),
        };
        if buf.len().saturating_add(n) > MAX_STDIN_BYTES {
            trap_abort(&format!(
                "lumi: stdin exceeds {MAX_STDIN_BYTES} bytes (soft cap; use smaller input)"
            ));
        }
...
#[no_mangle]
pub extern "C" fn lumi_str_starts_with(s: *mut u8, prefix: *mut u8) -> i64 {
...
#[no_mangle]
pub extern "C" fn lumi_str_ends_with(s: *mut u8, suffix: *mut u8) -> i64 {
    with_str_bytes(s, |bytes| {
...
#[no_mangle]
pub extern "C" fn lumi_str_contains(s: *mut u8, needle: *mut u8) -> i64 {
    with_str_bytes(s, |bytes| {
...
#[no_mangle]
pub extern "C" fn lumi_println_str(ptr: *const u8, len: u64) {
    let mut out = io::stdout().lock();
    if ptr.is_null() {
        let _ = writeln!(out);
        return;
...
#[no_mangle]
pub extern "C" fn lumi_println_bool(b: i8) {
    let mut out = io::stdout().lock();
...
#[no_mangle]
pub extern "C" fn lumi_println_cstr(ptr: *const u8) {
    let mut out = io::stdout().lock();
    if ptr.is_null() {
        let _ = writeln!(out);
        return;
...
#[no_mangle]
pub extern "C" fn lumi_alloc_string(ptr: *const u8, len: u64) -> *mut u8 {
    let dest = lumi_alloc(len, TYPE_STRING);
...
/// NUL-terminated C string copy of a Lumi String (for `foreign` String arguments).
#[no_mangle]
pub extern "C" fn lumi_string_cstr(s: *mut u8) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    if s.is_null() {
        let dest = lumi_alloc(1, TYPE_BYTES);
        unsafe {
            *dest = 0;
        }
        return dest;
    ...
    unsafe {
        ...
        if bytes.contains(&0) {
            trap_abort("lumi: String with interior NUL cannot convert to C string");
        }
        let nbytes = (n as u64)
            .checked_add(1)
            .filter(|&b| b <= u32::MAX as u64)
            .unwrap_or_else(|| trap_abort("lumi: cstr buffer too large"));
        let dest = lumi_alloc(nbytes, TYPE_BYTES);
...
/// Build a Lumi String from a NUL-terminated C string (foreign String returns).
#[no_mangle]
pub extern "C" fn lumi_cstr_to_string(cstr: *const u8) -> *mut u8 {
    if cstr.is_null() {
        return lumi_alloc_string(std::ptr::null(), 0);
    }
    unsafe {
        let mut n = 0usize;
        while *cstr.add(n) != 0 {
            n += 1;
            if n > 1 << 28 {
                trap_abort("lumi: cstr too long");
            }
        }
        lumi_alloc_string(cstr, n as u64)
    }
}
/// Print `x` as a heap String if it is one; ADTs via structural Show; otherwise Int.
#[no_mangle]
pub extern "C" fn lumi_println_auto(x: i64) {
    let p = x as *mut u8;
...
#[no_mangle]
pub extern "C" fn lumi_println_float(n: f64) {
    let mut out = io::stdout().lock();
    let _ = writeln!(out, "{n}");
}
#[no_mangle]
pub extern "C" fn lumi_match_fail() {
    trap_abort("lumi: non-exhaustive match");
}

/// Abort if `cond` is false (0). `msg` is a UTF-8 message (e.g. `path:line: assert failed`).
#[no_mangle]
pub extern "C" fn lumi_assert(cond: i64, msg: *const u8, msg_len: i64) {
    if cond == 0 {
        let text = if msg.is_null() || msg_len <= 0 {
            "lumi: assert failed".to_string()
        } else {
            let slice = unsafe { std::slice::from_raw_parts(msg, msg_len as usize) };
            match std::str::from_utf8(slice) {
                Ok(s) => format!("lumi: {s}"),
                Err(_) => "lumi: assert failed".to_string(),
            }
        };
        eprintln!("{text}");
        std::process::abort();
...
#[no_mangle]
pub extern "C" fn lumi_str_len(s: *mut u8) -> i64 {
    if s.is_null() {
        return 0;
    }
...
#[no_mangle]
pub extern "C" fn lumi_str_concat(a: *mut u8, b: *mut u8) -> *mut u8 {
    ...
    unsafe {
        ...
        let total = na
            ...
            .unwrap_or_else(|| trap_abort("lumi: string too large to concat"));
        let dest = lumi_alloc(total, TYPE_STRING);
        if dest.is_null() {
            trap_abort("lumi: str concat OOM");
        }
        if na > 0 {
...
#[no_mangle]
pub extern "C" fn lumi_str_trim(s: *mut u8) -> *mut u8 {
    ...
    with_str_bytes(s, |bytes| {
        ...
        lumi_alloc_string(owned.as_ptr(), owned.len() as u64)
    })
}

/// Substring `[start, end)` in byte offsets (clamped).
#[no_mangle]
pub extern "C" fn lumi_str_substring(s: *mut u8, start: i64, end: i64) -> *mut u8 {
    with_str_bytes(s, |bytes| {
...
#[no_mangle]
pub extern "C" fn lumi_str_to_lower(s: *mut u8) -> *mut u8 {
    with_str_bytes(s, |bytes| {
        let lower: Vec<u8> = bytes.iter().map(|b| b.to_ascii_lowercase()).collect();
        lumi_alloc_string(lower.as_ptr(), lower.len() as u64)
    })
}

#[no_mangle]
pub extern "C" fn lumi_str_to_upper(s: *mut u8) -> *mut u8 {
    with_str_bytes(s, |bytes| {
        let upper: Vec<u8> = bytes.iter().map(|b| b.to_ascii_uppercase()).collect();
        lumi_alloc_string(upper.as_ptr(), upper.len() as u64)
    })
}

/// Split `s` on separator Char (or raw codepoint). Returns List[String].
#[no_mangle]
pub extern "C" fn lumi_str_split(s: *mut u8, sep_ch: i64) -> *mut u8 {
    let _gc = GcInhibitGuard::enter();
    ...
    with_str_bytes(s, |bytes| {
        ...
        if sep.is_empty() {
            parts.push(lumi_alloc_string(bytes.as_ptr(), bytes.len() as u64));
        } else {
            ...
            while i + sep.len() <= bytes.len() {
                if &bytes[i..i + sep.len()] == sep.as_slice() {
                    ...
                    parts.push(lumi_alloc_string(slice.as_ptr(), slice.len() as u64));
                    i += sep.len();
                    start = i;
                } else {
                    i += 1;
...
#[no_mangle]
pub extern "C" fn lumi_trap_div0() {
    trap_abort("lumi: division by zero");
}

#[no_mangle]
pub extern "C" fn lumi_trap_overflow() {
    trap_abort("lumi: integer overflow");
}
