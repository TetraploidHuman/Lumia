...
/// Prints the Lumi call stack (pushed by codegen) then aborts.
...
thread_local! {
    ...
    /// Lumi-managed call stack for trap backtraces (nul-terminated name pointers).
...
pub(crate) fn header_layout(payload: usize) -> Layout {
    ...
    let Some(size) = header.checked_add(payload) else {
        trap_abort("lumi: allocation size overflow");
    };
    Layout::from_size_align(size, 8).unwrap_or_else(|_| {
        trap_abort("lumi: invalid allocation layout");
    })
}
