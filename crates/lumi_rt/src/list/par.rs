...
use super::core::{list_len_of, lumi_list_empty};
...
#[no_mangle]
pub extern "C" fn lumi_list_par_map(
    list: *mut u8,
    f: Option<extern "C" fn(i64) -> i64>,
    result_tid: u32,
) -> *mut u8 {
    let Some(f) = f else {
        trap_abort("lumi: list_par_map null function");
    };
    ...
    unsafe {
        if n <= 0 {
            return if list_elem_is_float(result_tid) {
                ...
                let dest = lumi_alloc(8, list_type_id(true));
                *(dest as *mut i64) = 0;
                dest
            } else {
                lumi_list_empty()
        ...
        let dest = lumi_alloc(list_payload_bytes(n), result_tid);
...
#[no_mangle]
pub extern "C" fn lumi_list_par_fold(
    list: *mut u8,
    init: i64,
    f: Option<extern "C" fn(i64, i64) -> i64>,
) -> i64 {
    let Some(f) = f else {
        trap_abort("lumi: list_par_fold null function");
    };
    let _gc = GcInhibitGuard::enter();
    let iota = !list.is_null() && list_tid(list) == TYPE_LIST_IOTA;
    let (n, iota_start, src_addr) = unsafe {
