...
pub(crate) fn lumi_ord_cmp(a: i64, b: i64) -> std::cmp::Ordering {
    ...
    if ha && hb {
        unsafe {
            ...
            if tid_base(ta) != tid_base(tb) {
                trap_abort("lumi: Ord operands have mixed heap types");
            }
            ...
            match tid_base(ta) {
                ...
                _ => trap_abort(&format!(
                    "lumi: type_id={ta} is not Ord (use Int/Float/Bool/String/Char or Ord ADT)"
                )),
            }
        }
    } else {
        trap_abort("lumi: cannot compare scalar with heap value under Ord");
    }
}
...
#[no_mangle]
pub extern "C" fn lumi_cmp(a: i64, b: i64) -> i64 {
    match lumi_ord_cmp(a, b) {
        std::cmp::Ordering::Less => -1,
        std::cmp::Ordering::Equal => 0,
        std::cmp::Ordering::Greater => 1,
    }
}
/// Stable content hash for Map/Set keys — must agree with `lumi_eq` (DESIGN §3.5.1).
pub fn lumi_hash(key: i64) -> u64 {
    hash_value(key, 0)
}
...
#[no_mangle]
pub extern "C" fn lumi_adt_tag(obj: *mut u8) -> i64 {
    if obj.is_null() {
        trap_abort("lumi: adt_tag on null");
    }
    unsafe { *(obj as *const i64) }
}

#[no_mangle]
pub extern "C" fn lumi_adt_field(obj: *mut u8, index: i64) -> i64 {
    if obj.is_null() || index < 0 {
        trap_abort(&format!("lumi: adt_field OOB (null or neg index={index})"));
    }
    unsafe {
        ...
        if words == 0 || (index as usize) + 1 >= words {
            ...
            trap_abort(&format!(
                "lumi: adt_field OOB index={index} nfields={nfields} tag={tag} bytes={} tid={tid} ptr=0x{bits:x}",
                (*h).size
            ));
        }
...
/// are not falsely shared. [`lumi_adt_set_field`] then installs the new value
/// (null old ⇒ no release).
unsafe fn adt_shallow_clone_heap(src: *mut u8, overwrite_mask: u64) -> *mut u8 {
    ...
    let dest = lumi_alloc(nbytes, (*h).type_id);
...
#[no_mangle]
pub extern "C" fn lumi_adt_ensure_unique(obj: *mut u8) -> *mut u8 {
    lumi_adt_ensure_unique_mask(obj, 0)
}

/// Like [`lumi_adt_ensure_unique`], but `overwrite_mask` skips nested retain on
/// fields that inplace `with` will rewrite (avoids RC≥2 on untouched siblings
/// when the product itself must clone).
#[no_mangle]
pub extern "C" fn lumi_adt_ensure_unique_mask(obj: *mut u8, overwrite_mask: u64) -> *mut u8 {
...
/// Drop one alias retain (e.g. with-temp), then [`lumi_adt_ensure_unique`].
/// Prefer plain `ensure_unique` when the with-temp Let was optimized away.
#[no_mangle]
pub extern "C" fn lumi_adt_ensure_unique_consume(obj: *mut u8) -> *mut u8 {
    lumi_adt_ensure_unique_consume_mask(obj, 0)
}

/// Consume with-temp retain, then unique-check with overwrite mask for `with`.
#[no_mangle]
pub extern "C" fn lumi_adt_ensure_unique_consume_mask(
    obj: *mut u8,
    overwrite_mask: u64,
) -> *mut u8 {
    use crate::common::cow_rc_drop_alias;
    cow_rc_drop_alias(obj, /*adt_ok=*/ true);
    lumi_adt_ensure_unique_mask(obj, overwrite_mask)
}

/// Write ADT field `index` (0-based): release old List/ADT, retain new, barrier.
#[no_mangle]
pub extern "C" fn lumi_adt_set_field(obj: *mut u8, index: i64, value: i64) {
    use crate::common::{value_rc_release_bits, value_rc_retain_bits};
    if obj.is_null() || index < 0 {
        trap_abort(&format!(
            "lumi: adt_set_field OOB (null or neg index={index})"
        ));
    }
    unsafe {
        let h = header_from_payload(obj);
        ...
        if words == 0 || (index as usize) + 1 >= words {
            trap_abort(&format!(
                "lumi: adt_set_field OOB index={index} words={words}"
            ));
        }
        ...
        *slot = value;
        if value_heap || !float_field {
            crate::gc::lumi_write_barrier(obj, (index + 1) as u32, value as *mut u8);
        }
    }
}
