...
use lumi_abi::{adt_show_kind, is_list_tid, is_map_tid, is_set_tid};

#[no_mangle]
pub extern "C" fn lumi_alloc_char(codepoint: i64) -> *mut u8 {
    let dest = lumi_alloc(8, TYPE_CHAR);
    if dest.is_null() {
        trap_abort("lumi: char OOM");
    }
    unsafe {
...
#[no_mangle]
pub extern "C" fn lumi_show(x: i64) -> *mut u8 {
    ...
    if is_heap_payload(p) {
        unsafe {
            ...
            if tid == TYPE_CHAR {
                ...
                return lumi_alloc_string(s.as_ptr(), s.len() as u64);
            }
    ...
    let s = x.to_string();
    lumi_alloc_string(s.as_ptr(), s.len() as u64)
}
...
pub(crate) fn show_value_bits(bits: i64, as_float: bool) -> String {
    if as_float {
        let s = lumi_show_float(f64::from_bits(bits as u64));
        return with_str_bytes(s, |b| String::from_utf8_lossy(b).into_owned());
    }
    let s = lumi_show(bits);
...
pub(crate) unsafe fn show_adt_masked_named(
    ...
    if words == 0 {
        return lumi_alloc_string(b"#()".as_ptr(), 3);
    }
    let tag = *base;
    ...
    for i in 1..words {
        ...
        let field = if crate::common::adt_float_slot(float_mask, i - 1) {
            lumi_show_float(f64::from_bits(bits as u64))
        } else {
            lumi_show(bits)
        };
...
#[no_mangle]
pub extern "C" fn lumi_show_adt(x: i64, float_mask: i64) -> *mut u8 {
    let p = x as *mut u8;
    if !is_heap_payload(p) {
        return lumi_show(x);
    }
...
#[no_mangle]
pub extern "C" fn lumi_show_adt_named(
    x: i64,
    float_mask: i64,
    names: *const *const u8,
    n_names: i64,
...
#[no_mangle]
pub extern "C" fn lumi_adt_set_float_mask(obj: *mut u8, float_mask: u64) {
    if obj.is_null() {
        return;
    }
    unsafe {
...
#[no_mangle]
pub extern "C" fn lumi_show_float(n: f64) -> *mut u8 {
    let s = n.to_string();
    lumi_alloc_string(s.as_ptr(), s.len() as u64)
}

#[no_mangle]
pub extern "C" fn lumi_show_bool(b: i8) -> *mut u8 {
