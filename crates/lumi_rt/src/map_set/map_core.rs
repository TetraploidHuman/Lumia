...
use crate::gc::{list_payload_bytes, lumi_alloc, mark, mark_value};
...
pub(crate) fn map_linear_nbytes(n: i64) -> usize {
    if n < 0 {
        trap_abort("lumi: negative map length");
    }
    (n as u64)
        .checked_mul(2)
        ...
        .map(|b| b as usize)
        .unwrap_or_else(|| trap_abort(&format!("lumi: map too large (n={n})")))
}
...
pub(crate) fn map_hash_nbytes(cap: usize) -> usize {
    ...
    cap.checked_mul(4)
        ...
        .unwrap_or_else(|| trap_abort(&format!("lumi: map hash table too large (cap={cap})")))
}

pub(crate) fn map_overlay_nbytes(dn: i64) -> usize {
    if dn < 0 {
        trap_abort("lumi: negative overlay delta");
    }
    (dn as u64)
        .checked_mul(2)
        ...
        .filter(|&b| b <= u32::MAX as u64)
        .map(|b| b as usize)
        .unwrap_or_else(|| trap_abort(&format!("lumi: map overlay too large (dn={dn})")))
}
...
pub(crate) fn map_count(map: *mut u8) -> i64 {
    ...
    unsafe {
        if map_is_overlay(map) {
            ...
            // Overlay may sit on a Float-keyed parent — use key_eq (±0), not bit `lumi_eq`.
...
pub(crate) unsafe fn map_materialize(map: *mut u8) -> *mut u8 {
    ...
    let mut dest = if map_is_hash(parent) || map_count(parent) + dn as i64 > MAP_SMALL_MAX {
        ...
        let out = lumi_alloc(nbytes, map_tid(parent));
...
pub(crate) unsafe fn map_clone_hash_upsert_or_linear(map: *mut u8, key: i64, val: i64) -> *mut u8 {
    if map_is_hash(map) {
        ...
        // linear upsert (same as lumi_map_set linear branch)
...
pub(crate) fn alloc_adt(tag: i64, fields: &[i64]) -> *mut u8 {
    ...
    let dest = lumi_alloc(nbytes, TYPE_ADT);
    if dest.is_null() {
        trap_abort("lumi: adt OOM");
    }
    unsafe {
        let dst = dest as *mut i64;
        *dst = tag;
    ...
    for _ in 0..cap {
        ...
        if st == MAP_ST_EMPTY || st == MAP_ST_TOMB {
            ...
            *cell.add(2) = MAP_ST_FULL;
            if !float_keys {
                crate::lumi_write_barrier(dest, order_i as u32, key as *mut u8);
            }
            if !map_float_vals(dest) {
                crate::lumi_write_barrier(dest, order_i as u32, val as *mut u8);
            }
...
pub(crate) unsafe fn map_hash_upsert_build(dest: *mut u8, key: i64, val: i64) -> bool {
    if let Some(slot) = map_hash_find_slot(dest, key) {
        ...
        *cell.add(1) = val; // last wins
        if !map_float_vals(dest) {
            crate::lumi_write_barrier(dest, slot as u32, val as *mut u8);
        }
        return false;
    }
    let base = dest as *mut i64;
