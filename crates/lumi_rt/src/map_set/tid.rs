...
use crate::eq::lumi_eq;
use crate::hash_ord::lumi_hash;
use lumi_abi::{
...
pub(crate) fn map_is_assoc(map: *mut u8) -> bool {
    ...
    lumi_abi::map_tid_is_assoc(unsafe { (*header_from_payload(map)).type_id })
}
...
pub(crate) fn set_is_assoc(set: *mut u8) -> bool {
    ...
    lumi_abi::set_tid_is_assoc(unsafe { (*header_from_payload(set)).type_id })
}
...
pub(crate) fn map_float_keys(map: *mut u8) -> bool {
    ...
    lumi_abi::map_key_is_float(unsafe { (*header_from_payload(map)).type_id })
}
...
pub(crate) fn map_float_vals(map: *mut u8) -> bool {
    ...
    lumi_abi::map_val_is_float(unsafe { (*header_from_payload(map)).type_id })
}
...
pub(crate) fn set_float_elems(set: *mut u8) -> bool {
    ...
    lumi_abi::set_elem_is_float(unsafe { (*header_from_payload(set)).type_id })
}
...
pub(crate) fn ensure_map_f64(map: *mut u8) -> *mut u8 {
    ensure_empty_float_retag(
        ...
        |tid, map| {
            if !is_map_tid(tid) {
                return Err(format!("lumi: ensure_map_f64 on type_id={tid}"));
            }
            if map_count(map) != 0 {
                return Err("lumi: ensure_map_f64 on non-empty Int-key map".into());
            }
...
pub(crate) fn ensure_map_vf64(map: *mut u8) -> *mut u8 {
    ensure_empty_float_retag(
        ...
        |tid, map| {
            if !is_map_tid(tid) {
                return Err(format!("lumi: ensure_map_vf64 on type_id={tid}"));
            }
            if map_count(map) != 0 {
                return Err("lumi: ensure_map_vf64 on non-empty non-Float-value map".into());
            }
...
pub(crate) fn ensure_set_f64(set: *mut u8) -> *mut u8 {
    ensure_empty_float_retag(set, abi_set_type_id(true, false), tid_f_key, |tid, set| {
        if !is_set_tid(tid) {
            return Err(format!("lumi: ensure_set_f64 on type_id={tid}"));
        }
        if unsafe { *(set as *const i64) } != 0 {
            return Err("lumi: ensure_set_f64 on non-empty Int-elem set".into());
        }
        Ok(tid_with_f_key(tid))
    })
}
...
#[no_mangle]
pub extern "C" fn lumi_ensure_map_f64(map: *mut u8) -> *mut u8 {
    ensure_map_f64(map)
}

#[no_mangle]
pub extern "C" fn lumi_ensure_map_vf64(map: *mut u8) -> *mut u8 {
    ensure_map_vf64(map)
}

#[no_mangle]
pub extern "C" fn lumi_ensure_set_f64(set: *mut u8) -> *mut u8 {
    ensure_set_f64(set)
}
