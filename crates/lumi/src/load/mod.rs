...
pub struct LoadedProgram {
    ...
    pub module: Module,
    /// Linker flags from `Lumi.toml` `package.link`.
    pub link_args: Vec<String>,
    /// From `Lumi.toml` `package.trust_foreign_pure`.
...
pub fn load_program_with_overlays(
    ...
    if let Some(manifest_path) = crate::pkg::find_manifest(&entry) {
        ...
        if !m.dependencies.is_empty() && !lock_path.is_file() {
            bail!(
                "dependencies declared in {} but {} is missing (run `lumi pkg lock`)",
