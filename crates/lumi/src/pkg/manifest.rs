//! Lumi package manifest (`Lumi.toml`).
...
/// Walk parents from `start` looking for `Lumi.toml`.
...
pub fn write_manifest(path: &Path, m: &Manifest) -> Result<()> {
    let s = toml::to_string_pretty(m).context("serialize Lumi.toml")?;
...
pub(super) fn resolve_dep_path(root: &Path, name: &str, spec: &DepSpec) -> Result<PathBuf> {
    ...
    if !path.exists() {
        bail!(
            "dependency `{name}` path {} does not exist (run `lumi pkg lock` after vendoring)",
            path.display()
        );
    }
...
fn collect_dep_roots(
    ...
    for (name, spec) in &m.dependencies {
        ...
        // Transitive: if dep has its own Lumi.toml, pull its deps too.
...
pub fn add_path_dep(manifest_path: &Path, name: &str, dep_path: &str) -> Result<()> {
    ...
    // Validate before writing so a rejected path cannot corrupt Lumi.toml.
