//! Lumi package manifest (`Lumi.toml`) + lockfile (`Lumi.lock`).
...
mod tests {
    ...
    fn add_path_dep_rejects_dotdot_without_writing() {
        let dir = std::env::temp_dir().join(format!("lumi_pkg_add_{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let manifest = dir.join("Lumi.toml");
        fs::write(
            &manifest,
            r#"[package]
name = "t"
...
"#,
    ...
    fn link_args_resolve_relative_to_package_root() {
        let dir = std::env::temp_dir().join(format!("lumi_pkg_link_{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
