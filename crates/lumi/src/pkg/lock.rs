...
pub fn write_lockfile(path: &Path, lock: &Lockfile) -> Result<()> {
    let s = toml::to_string_pretty(lock).context("serialize Lumi.lock")?;
...
/// Build a lockfile from the manifest (path pins; versions from dep `Lumi.toml` when present).
...
fn lock_deps_recursive(
    ...
    for (name, spec) in &m.dependencies {
        ...
        let dep_manifest = abs.join("Lumi.toml");
...
fn read_package_version(dep_root: &Path) -> Option<String> {
    let cand = if dep_root.is_file() {
        ...
        dep_root.join("Lumi.toml")
    };
    load_manifest(&cand).ok().map(|m| m.package.version)
}

/// Verify `Lumi.lock` against the manifest: every dep is present with matching path/version.
pub fn verify_lockfile(manifest_path: &Path, m: &Manifest, lock: &Lockfile) -> Result<()> {
    ...
    for exp in &expected.package {
        ...
        let Some(got) = lock.package.iter().find(|p| p.name == exp.name) else {
            bail!(
                "Lumi.lock missing dependency `{}` (run `lumi pkg lock`)",
                exp.name
            );
        };
        if got.path != exp.path {
            bail!(
                "Lumi.lock path for `{}` is `{}`, expected `{}` (run `lumi pkg lock`)",
                exp.name,
                got.path,
                exp.path
            );
        ...
        if got.version != exp.version {
            bail!(
                "Lumi.lock version for `{}` is `{}`, expected `{}` (run `lumi pkg lock`)",
                exp.name,
                got.version,
                exp.version
            );
