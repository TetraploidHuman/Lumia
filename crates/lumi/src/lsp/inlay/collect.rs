...
use lumi_hir::{for_each_expr, Expr, Item};
use lumi_syntax::{byte_to_line_col, line_starts, Span};
use lumi_ty::{display_type, expr_span, pretty_type_with, subst_num_vars, var_names_for, Type};
...
fn pos_json(src: &str, byte: u32) -> Value {
    ...
    let (line, col) = byte_to_line_col(&starts, lumi_syntax::BytePos(byte));
    json!({
