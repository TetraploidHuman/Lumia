...
fn lumi_bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_lumi"))
}
...
fn build_and_run(rel: &str, release: bool) -> Vec<String> {
    ...
    assert!(
        status.success(),
        "lumi build failed for {rel} release={release}: {status}"
    );
...
fn opt_sr_correctness_matches_rt_oracles() {
    ...
    assert_eq!(
        ...
        lumi_rt::lumi_count_primes(20_000)
    );
    assert_eq!(
        got[1].parse::<i64>().unwrap(),
        lumi_rt::lumi_matmul_affine_checksum(80, 1_000_003)
    );
    assert_eq!(
        got[2].parse::<i64>().unwrap(),
        lumi_rt::lumi_mandelbrot_checksum(40)
    );
    assert_eq!(
        got[3].parse::<i64>().unwrap(),
        lumi_rt::lumi_mandelbrot_checksum(450)
    );
    assert_eq!(
        got[4].parse::<i64>().unwrap(),
        lumi_rt::lumi_collatz_total(20_000)
    );
    assert_eq!(
        got[5].parse::<i64>().unwrap(),
        lumi_rt::lumi_collatz_strided(1, 30_000, 3)
    );
    assert_eq!(
        got[7].parse::<i64>().unwrap(),
        lumi_rt::lumi_affine2_rem_sum(400, 131, 17, 1, 10_007)
    );
    assert_eq!(got[8].parse::<i64>().unwrap(), lumi_rt::lumi_gcd_sum(200));
    assert_eq!(
        got[9].parse::<i64>().unwrap(),
        lumi_rt::lumi_divisor_sum(100_000)
    );
    assert_eq!(
        got[10].parse::<i64>().unwrap(),
        lumi_rt::lumi_product_rem_sum(200, 10_007)
    );
    assert_eq!(
        got[11].parse::<i64>().unwrap(),
        lumi_rt::lumi_product_rem_sum(12_000, 10_007)
    );
    assert_eq!(
        got[15].parse::<i64>().unwrap(),
        lumi_rt::lumi_affine1_rem_sum(20_000, 131, 17, 10_007)
    );
}
