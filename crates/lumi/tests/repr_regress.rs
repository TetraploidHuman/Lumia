//! Representation-off regression: disabling escape/repr_select must not change semantics.
#![cfg(feature = "codegen")]
...
fn out_dir() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("lumi_repr_reg_{}", std::process::id()));
...
fn assert_repr_off_matches_stock(rel: &str, disables: PassDisables, tag: &str, release: bool) {
    ...
    assert_eq!(
        ...
        "{rel}: repr-off output diverged from stock\n stock: {stock_out:?}\n caps:  {capped_out:?}"
    );
}

#[test]
fn repr_select_off_small_list() {
    assert_repr_off_matches_stock(
...
#[test]
fn escape_and_repr_off_cow_nested() {
    assert_repr_off_matches_stock(
        "examples/cow_nested_list.lm",
        PassDisables {
            no_escape: true,
            ..Default::default()
        },
...
#[test]
fn repr_select_off_release_pe_list() {
    assert_repr_off_matches_stock(
        "examples/pe_list_len_get.lm",
        PassDisables {
            no_repr_select: true,
            ..Default::default()
        },
