...
pub(crate) fn e2e_out_dir() -> PathBuf {
    ...
    let out_dir = std::env::temp_dir().join(format!("lumi_e2e_{}", std::process::id()));
...
pub(crate) fn run_example_build(
    ...
    assert!(status.success(), "lumi build failed for {rel}: {status}");
...
/// Run `lumi check` and assert failure + required diagnostic substrings.
