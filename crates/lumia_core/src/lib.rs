...
pub use sr_pattern::{
    ...
    has_float_binop_with_const, header_ge_const, header_gt_const, header_gt_eq, header_le_bound,
    header_le_const, header_lt_bound, header_lt_const, header_name_sq_cmp, header_name_sq_le_name,
    is_add_name_plus_any, is_add_name_plus_name, is_affine_ik1, is_affine_kj1, is_list_get,
    is_list_set, is_name_add_const, is_name_div_const, is_name_mul_const,
    is_name_mul_const_plus_const, is_name_mul_name, is_name_ne_zero, is_name_rem_eq_const,
    is_nontrivial_add_or_sub, is_nontrivial_arith, is_rem, is_small_factor_mul_nonneg, is_unit_inc,
    is_unit_inc_value, is_unit_step, local_is_zero_or_false, name_ne_zero, name_of, rem_const_mod,
    rem_eq_zero_names, rem_eq_zero_operands, same_local,
};
pub use value_ty::{
    builtin_result_may_heap, infer_value_ty, infer_value_ty_ctx, join_abi_tys, join_fixed_ty,
    list_par_map_elem_ty, prefer_concrete_heap_ty, type_may_heap, value_alloc_may_heap,
    CodegenTypeTables, HeapMay, HeapPolicy, InferValueCtx, JoinAbiKind,
};
pub use visit::{
    block_calls, block_has_io, collect_alloc_closure_caps, collect_alloc_closure_env_funs,
    collect_assigned_names, collect_assigns, collect_call_names_in, collect_closure_cap_funrefs,
    collect_defined_locals, collect_loops, collect_slot_names, collect_ssa_live_refs,
    collect_uses_in_value, count_ops, first_direct_loop, for_each_block_dfs,
    for_each_ctrl_nested_block_mut, for_each_let_value, for_each_let_value_ctrl, for_each_local,
    for_each_local_mut, for_each_nested_block, for_each_nested_block_mut, for_each_op_value_mut,
    has_assign_or_name, has_early_return, local_let_matches, map_value_locals, max_local_in_block,
    max_local_in_fun, max_local_in_value, resolve_module_call_fun_ids, rewrite_block_locals,
