...
"#,
    ...
    /// After `f(xs, eps)` where eps is Float, rewrite must still see `xs` as
    /// `List[Float]` so subsequent specialized callees are rewritten (not left
    /// on the generic with Int `*` on IEEE bits).
    #[test]
