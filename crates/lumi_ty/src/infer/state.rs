...
pub(crate) struct TraitState {
    ...
    pub(crate) ufcs_rewrites: HashMap<lumi_syntax::Span, String>,
}
...
pub(crate) struct AltReturnState {
    ...
    pub(crate) alt_kinds: HashMap<lumi_syntax::Span, AltKind>,
    /// Ambiguous `.field` → (adt_name, idx) once receiver is known.
    pub(crate) product_field_rewrites: HashMap<lumi_syntax::Span, (String, i64)>,
    /// Deferred `with` → product type name.
    pub(crate) with_rewrites: HashMap<lumi_syntax::Span, String>,
}
