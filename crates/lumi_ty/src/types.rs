...
impl From<lumi_syntax::LocatedError> for TypeError {
    fn from(e: lumi_syntax::LocatedError) -> Self {
        TypeError::Located {
            span: e.span,
            message: e.message,
        }
...
impl TypeError {
    pub fn span(&self) -> Option<lumi_syntax::Span> {
        match self {
...
pub(crate) fn at(span: lumi_syntax::Span, msg: impl Into<String>) -> TypeError {
    TypeError::Located {
        span,
        message: msg.into(),
    }
...
pub fn expr_span(e: &Expr) -> lumi_syntax::Span {
    match e {
        Expr::Int(_, s)
        | Expr::Float(_, s)
        | Expr::Bool(_, s)
...
pub(crate) fn locate(span: lumi_syntax::Span, err: TypeError) -> TypeError {
    match err {
...
pub struct TypedModule {
    ...
    pub type_at: Vec<(lumi_syntax::Span, Type)>,
    /// Top-level / local binding name → declaration span (for go-to-def).
    pub decls: HashMap<String, lumi_syntax::Span>,
}
