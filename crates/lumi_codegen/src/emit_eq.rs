...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn emit_show_override(
        ...
        let mangled = lumi_hir::mangle_trait_method("Show", adt_name, "show");
    ...
    pub(crate) fn emit_eq_override(
        ...
        let mangled = lumi_hir::mangle_trait_method("Eq", adt_name, "eq");
        let Some(fv) = self.funs.functions.get(&mangled).copied() else {
            return Ok(None);
        };
    ...
    pub(crate) fn emit_less_override(
        ...
        let mangled = lumi_hir::mangle_trait_method("Ord", adt_name, "less");
        let Some(fv) = self.funs.functions.get(&mangled).copied() else {
            return Ok(None);
        };
        let call = crate::error::llvm(self.llvm.builder.build_call(
    ...
    /// Typed `==` for ADTs with Float fields and fallback to `lumi_eq`.
    pub(crate) fn emit_value_eq(
        ...
        // Int/Bool/Unit: bit identity (matches `lumi_eq` scalar short-circuit).
        ...
        if let Some(name) = Self::adt_method_name(lt, rt) {
            // Hash ADTs use `lumi_eq` for Map/Set keys — keep `==` on the same path
    ...
    pub(crate) fn emit_typed_adt_eq(
        ...
        let f = self.runtime_fn("lumi_adt_eq")?;
    ...
    pub(crate) fn emit_typed_adt_show(
        ...
        let f = self.runtime_fn("lumi_show_adt")?;
        let call = crate::error::llvm(self.llvm.builder.build_call(
            f,
            &[i.into(), mask_v.into()],
            "show_adt",
    ...
    /// `lumi_show_adt_named(obj, mask, names_ptr, n)`.
    pub(crate) fn emit_show_adt_named(
        ...
        let f = self.runtime_fn("lumi_show_adt_named")?;
        let call = crate::error::llvm(self.llvm.builder.build_call(
            f,
