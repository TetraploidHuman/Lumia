...
use lumi_abi::{adt_type_id, list_type_id, map_type_id, set_type_id};
use lumi_core::Local;
use lumi_ty::Type;
...
impl<'ctx> Codegen<'ctx> {
    pub(crate) fn emit_value_alloc_list(
        ...
        if elems.is_empty() {
            if float_elems {
                let ens = self.runtime_fn(lumi_abi::ENSURE_LIST_F64)?;
                let f = self.runtime_fn("lumi_list_empty")?;
                let __call1 =
    ...
    pub(crate) fn emit_value_alloc_set(
        ...
        if elems.len() > 8 && !no_hash {
            ...
            let f = self.runtime_fn("lumi_set_finish")?;
            let __call4 =
    ...
    pub(crate) fn emit_value_alloc_map(
        ...
        let no_hash =
            matches!(repr, lumi_core::MapRepr::AssocList) || !self.key_type_has_hash(&key_ty);
        ...
        let alloc = self.runtime_fn("lumi_alloc")?;
        ...
        let ptr = if !no_hash && (n_pairs > 8 || matches!(repr, lumi_core::MapRepr::HashOrdered)) {
            let f = self.runtime_fn("lumi_map_finish")?;
    ...
    fn emit_adt_with_inplace(
        ...
        let ensure = self.runtime_fn("lumi_adt_ensure_unique_consume_mask")?;
        ...
        .into_pointer_value();
        let setf = self.runtime_fn("lumi_adt_set_field")?;
