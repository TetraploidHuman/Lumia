...
impl<'ctx> Codegen<'ctx> {
    pub(crate) fn emit_checked_neg(
        ...
        let trap = self.runtime_fn("lumi_trap_overflow")?;
    ...
    pub(crate) fn emit_checked_div_rem(
        ...
        if let Some(c) = r.get_sign_extended_constant() {
            if c == 0 {
                let trap = self.runtime_fn("lumi_trap_div0")?;
            ...
            if c == -1 {
                ...
                self.llvm.builder.position_at_end(trap_bb);
                let t1 = self.runtime_fn("lumi_trap_overflow")?;
        ...
        let t0 = self.runtime_fn("lumi_trap_div0")?;
    ...
    fn try_emit_num_override(
        ...
        let mangled = lumi_hir::mangle_trait_method("Num", &name, method);
    ...
    fn emit_ord_compare(
        ...
        if let Some(name) = Self::adt_method_name(lt, rt) {
            if self
                .funs
                .functions
                .contains_key(&lumi_hir::mangle_trait_method("Ord", &name, "less"))
            {
        ...
        let f = self.runtime_fn("lumi_cmp")?;
        let call = crate::error::llvm(self.llvm.builder.build_call(
            f,
            &[l.into(), r.into()],
            "cmp",
