...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn emit_println_value(
        ...
        match arg_ty {
            ...
            Type::Float => {
                ...
                self.call_rt_void("lumi_println_float", &[f.into()], "println_float")?;
            }
            Type::Bool => {
                ...
                self.call_rt_void("lumi_println_bool", &[b.into()], "println_bool")?;
            }
            Type::Adt { name, params } => {
                ...
                if let Some(ptr) = ptr {
                    let len = self
                        .call_rt_basic("lumi_str_len", &[ptr.into()], "show_len")?
                        .into_int_value();
                    self.call_rt_void(
                        "lumi_println_str",
                        &[ptr.into(), len.into()],
                    ...
                    let i = self.coerce_i64(arg)?;
                    self.call_rt_void("lumi_println_auto", &[i.into()], "println")?;
                }
            }
            _ => {
                let i = self.coerce_i64(arg)?;
        ...
        match arg_ty {
            Type::Unit => {
                ...
                let ptr = self
                    ...
                    .as_pointer_value();
                let fun = self.runtime_fn("lumi_alloc_string")?;
            ...
            Type::Float => {
                ...
                let fun = self.runtime_fn("lumi_show_float")?;
                Ok(crate::error::llvm(self.llvm.builder.build_call(
                    fun,
                    &[f.into()],
                    "show_float",
            ...
            Type::Bool => {
                ...
                let fun = self.runtime_fn("lumi_show_bool")?;
                Ok(
    ...
    fn emit_show_auto(&mut self, arg: BasicValueEnum<'ctx>) -> Result<PointerValue<'ctx>> {
        let i = self.coerce_i64(arg)?;
        let fun = self.runtime_fn("lumi_show")?;
        Ok(
