//! LLVM codegen via inkwell (LLVM 21). Links against `lumi_rt`.
...
use lumi_core::{CoreFun, CoreModule, Local, Op, Value};
use lumi_ty::Type;
...
fn emit_llvm_module<'ctx>(
    ...
    for f in &core.functions {
        ...
        let fv = if let Some(sym) = &f.external {
            let runtime_abi = sym.starts_with("lumi_");
            // Prefer the declaration from `declare_runtime` when present so
            // LLVM types match `lumi_rt` (e.g. Bool as i64, List as ptr).
...
/// Register ADT constructor-name tables used by recursive `lumi_show`.
fn emit_adt_show_registration<'ctx>(
    ...
    let Some(reg) = module.get_function("lumi_adt_register_show") else {
        return Ok(());
    };
    let i32_ty = context.i32_type();
...
fn emit_trait_dict_registration<'ctx>(
    ...
    let Some(reg) = module.get_function("lumi_dict_register") else {
        return Ok(());
    };
    // (prefix, trait_id) — ids match lumi_rt::TRAIT_*; prefix is
...
impl<'ctx> Codegen<'ctx> {
    ...
    /// Look up a `lumi_rt` symbol declared by [`declare_runtime`].
    ///
...
/// ABI for `foreign` symbols implemented in `lumi_rt` (`lumi_*`):
...
mod tests {
    ...
    fn test_opts() -> CodegenOptions {
        CodegenOptions {
            release: false,
            output: PathBuf::from("/tmp/lumi_codegen_test"),
            emit_ir: false,
    ...
    fn emit_memo_tf_has_lookup_and_store() {
        ...
        assert!(
            ir.contains("lumi_memo_l2_lookup"),
            "expected lumi_memo_l2_lookup in memo_tf IR"
        );
        assert!(
            ir.contains("lumi_memo_l2_store"),
            "expected lumi_memo_l2_store in memo_tf IR"
        );
    }
    ...
    fn emit_float_map_keys_verifies() {
        ...
        assert!(
            ir.contains("lumi_ensure_map_f64") || ir.contains("lumi_map"),
