...
use lumi_core::{Local, MemoTf, Value};
use lumi_ty::Type;
...
pub(crate) struct FunTables<'ctx> {
    ...
    /// `foreign` symbols that use the `lumi_rt` object ABI (no String↔cstr).
    ...
    /// Stable Show-kind ids (`≥ 1`) packed into ADT `type_id` for recursive `lumi_show`.
