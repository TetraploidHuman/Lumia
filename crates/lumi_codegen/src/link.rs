...
pub(crate) fn link_executable(
    ...
    // `lumi_rt` is a Rust staticlib: pull in the host libs Rust std needs.
    // (Matches `cargo rustc -p lumi_rt -- --print=native-static-libs`.)
    if cfg!(target_os = "windows") {
        ...
        cmd.arg("-lpthread")
            ...
            .arg("-lrt")
            .arg("-lutil");
        // Drop unused `lumi_rt` / Rust-std objects. Cuts Release text ~3× and
...
/// Locate `liblumi_rt.a` / `lumi_rt.lib` in target dir.
pub fn find_runtime_lib_prefer(target_dir: &Path, release: bool) -> Result<PathBuf> {
    ...
    for p in profiles {
        for name in ["liblumi_rt.a", "lumi_rt.lib", "lumi_rt.dll.lib"] {
    ...
    for name in ["liblumi_rt.a", "lumi_rt.lib"] {
    ...
    if let Some(c) = found_fallback {
        eprintln!(
            "warning: linking {} lumi_rt into a {} build ({}); run `cargo build -p lumi_rt{}` for a matching runtime",
            fallback,
            preferred,
            c.display(),
    ...
    bail!(
        "liblumi_rt.a / lumi_rt.lib not found under {} — run `cargo build -p lumi_rt` first",
        target_dir.display()
    )
}
