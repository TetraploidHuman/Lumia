...
impl<'ctx> Codegen<'ctx> {
    ...
    /// Codegen tables for [`lumi_core::infer_value_ty_ctx`] / ParMap elem typing.
    pub(crate) fn infer_ctx(&self) -> lumi_core::InferValueCtx<'_> {
        lumi_core::InferValueCtx::full(
            &self.frame.local_tys,
            lumi_core::CodegenTypeTables {
    ...
    pub(crate) fn ensure_funref_ptr(
        ...
        let fail = self.runtime_fn("lumi_match_fail")?;
        crate::error::llvm(
            self.llvm
                .builder
    ...
    /// because `lumi_alloc` returns a young object.
    #[allow(dead_code)]
