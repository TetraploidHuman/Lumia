...
impl<'ctx> Codegen<'ctx> {
    /// Coerce a Lumi local to a C ABI argument for `foreign` calls.
    pub(crate) fn emit_c_abi_arg(
        ...
        match ty {
            ...
            Type::String => {
                ...
                let f = self.runtime_fn("lumi_string_cstr")?;
                let call =
        ...
        match ret {
            ...
            Type::String => {
                let p = call
                    ...
                    .into_pointer_value();
                let f = self.runtime_fn("lumi_cstr_to_string")?;
    ...
    /// `lumi_rt` object ABI: String/List as heap pointers (no cstr conversion).
