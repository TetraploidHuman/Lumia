...
impl<'ctx> Codegen<'ctx> {
    pub(crate) fn emit_function(&mut self, fun: &CoreFun) -> Result<()> {
        ...
        if self.nsw_iv {
            ...
            self.frame.safe_divisor_locals =
                crate::nsw_iv::collect_safe_divisor_locals(&fun.body);
            self.frame.nonneg_iv_load_locals =
                crate::nsw_iv::collect_nonneg_iv_load_locals(&fun.body);
        } else {
            self.frame.nsw_binop_locals = Default::default();
            self.frame.safe_divisor_locals = Default::default();
            self.frame.nonneg_iv_load_locals = Default::default();
        }
