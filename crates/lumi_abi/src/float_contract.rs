...
//! 4. **Ensure**: codegen calls `lumi_ensure_*_f64` before writing Float into a
...
//!    bit equality in `lumi_eq`. Scalar `==` on Float is emitted as `fcmp` in
//!    codegen and does not go through `lumi_eq`. Nested Float-as-bits without a
...
pub const ENSURE_MAP_F64: &str = "lumi_ensure_map_f64";
/// Runtime symbol for Float-value tagging on a map.
pub const ENSURE_MAP_VF64: &str = "lumi_ensure_map_vf64";
/// Runtime symbol for Float-elem tagging on a set.
pub const ENSURE_SET_F64: &str = "lumi_ensure_set_f64";
/// Runtime symbol for Float-elem tagging on a list.
pub const ENSURE_LIST_F64: &str = "lumi_ensure_list_f64";
