//! Shared ABI constants for the Lumi runtime and compiler.
//!
//! Codegen emits these `type_id` values into object headers; `lumi_rt` interprets
...
///
/// C ABI entry points remain `lumi_memo_l2_*` (frozen). Use `MEMO_TF_*` in Rust.
...
/// Repo root given a workspace crate's `CARGO_MANIFEST_DIR` (`crates/<name>` → `…/Lumi`).
#[inline]
