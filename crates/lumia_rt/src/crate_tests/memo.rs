...
fn memo_tf_hit_miss() {
    ...
    let mut out = 0i64;
    assert_eq!(
        unsafe { lumia_memo_l2_lookup(0, 1, 42, 0, 0, 0, &mut out) },
        0
    );
    ...
    assert_eq!(out, 99);
    assert_eq!(
        unsafe { lumia_memo_l2_lookup(0, 1, 7, 0, 0, 0, &mut out) },
        0
    );
    // 4-arg key
    lumia_memo_l2_store(1, 4, 1, 2, 3, 4, 77);
    assert_eq!(
        unsafe { lumia_memo_l2_lookup(1, 4, 1, 2, 3, 4, &mut out) },
        1
    );
    assert_eq!(out, 77);
    assert_eq!(
        unsafe { lumia_memo_l2_lookup(1, 4, 1, 2, 3, 5, &mut out) },
        0
    );
...
fn memo_idx_hit_miss() {
    ...
    assert_eq!(
        unsafe { lumia_memo_idx_lookup(0, MEMO_IDX_CAP as i64, &mut out) },
        0
    );
