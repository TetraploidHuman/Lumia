...
extern "C" fn send_n(env: i64) -> i64 {
    ...
    unsafe { lumia_channel_send(env as *mut u8, NEXT_SEND.fetch_add(1, Ordering::Relaxed)) };
    0
}
...
extern "C" fn echo_worker(env: i64) -> i64 {
    ...
    for _ in 0..200 {
        let x = unsafe { lumia_channel_recv(pair.req) };
        unsafe { lumia_channel_send(pair.resp, x) };
    }
    0
}
...
extern "C" fn ping_client(env: i64) -> i64 {
    ...
    let mut sum = 0i64;
    for i in 1..=200 {
        unsafe { lumia_channel_send(pair.req, i) };
        sum += unsafe { lumia_channel_recv(pair.resp) };
    }
    sum
}
...
fn stress_channel_fan_in() {
    ...
    let mut sum = 0i64;
    for _ in 0..N {
        sum += unsafe { lumia_channel_recv(ch) };
    }
