...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn emit_value_indirect_call(
        ...
        let fn_ptr2 = self
            ...
            .unwrap();
        let mut clos_param_tys: Vec<BasicMetadataTypeEnum> = vec![self.i64_ty.into()];
        for _ in args.iter() {
    ...
    pub(crate) fn emit_value_closure_cap(
        ...
        let loaded = self.builder.build_load(self.i64_ty, slot, "cap").unwrap();
        if as_float {
            Ok(self
                .builder
                .build_bit_cast(loaded.into_int_value(), self.context.f64_type(), "cap_f64")
                .unwrap())
        } else {
            Ok(loaded)
        }
