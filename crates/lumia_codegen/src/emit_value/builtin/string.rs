        ...
        match name {
            Builtin::StrTrim | Builtin::StrToLower | Builtin::StrToUpper => {
                ...
                let fname = name
                    .info()
                    .runtime_symbol
                    .expect("string unary builtins have runtime symbols");
                ...
                Ok(self
                    .llvm
                    .builder
                    .build_ptr_to_int(ptr, self.llvm.i64_ty, "str_i64")
                    .unwrap()
                    .into())
            }
            Builtin::StrSplit => {
                ...
                let ptr_ty = self.llvm.context.ptr_type(AddressSpace::default());
                let s = crate::error::llvm(self.llvm.builder.build_int_to_ptr(s_i, ptr_ty, "str"))?;
                let f = self.runtime_fn(Builtin::StrSplit.info().runtime_symbol.unwrap())?;
                let call = self
                    .llvm
                    .builder
                ...
                Ok(self
                    .llvm
                    .builder
                    .build_ptr_to_int(ptr, self.llvm.i64_ty, "split_i64")
                    .unwrap()
                    .into())
            }
            Builtin::StrSubstring => {
                ...
                let ptr_ty = self.llvm.context.ptr_type(AddressSpace::default());
                let s = crate::error::llvm(self.llvm.builder.build_int_to_ptr(s_i, ptr_ty, "str"))?;
                let f = self.runtime_fn(Builtin::StrSubstring.info().runtime_symbol.unwrap())?;
                let call = self
                    .llvm
                    .builder
                ...
                Ok(self
                    .llvm
                    .builder
                    .build_ptr_to_int(ptr, self.llvm.i64_ty, "substr_i64")
                    .unwrap()
                    .into())
            }
