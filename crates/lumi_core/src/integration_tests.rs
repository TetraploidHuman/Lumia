//! Integration tests for the core pipeline (mono, lift, println specialization).

use crate::{compile_source_to_core, CoreModule, Op, Value};
use lumi_hir::lower_module;
use lumi_syntax::parse_module;
use lumi_ty::{infer_module, Type};

fn main_calls(core: &CoreModule) -> Vec<String> {
...
fn main_println_calls(core: &CoreModule) -> Vec<String> {
    let main = core.functions.iter().find(|f| f.is_main).expect("main");
    main.body
        .ops
        .iter()
...
"#,
    ...
    let calls = main_println_calls(&core);
    assert!(
